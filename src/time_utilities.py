...
def convert_date2num(time_in) -> Any:
    return nc.date2num(
        time_in, "hours since 1900-01-01 00:00:00.0", calendar="gregorian"
    )
...
def create_list_of_dates(config: dict) -> pandas.date_range:
    ...
    if config["period_detection"] == 24:
        ...
        sys.exit(1)
    listofdates = pandas.date_range(
        start=datetime_startdate, end=datetime_enddate, freq=freq
    )
    return listofdates


def get_startendtimes_from_config(config: dict) -> tuple:
    """
