...
def plot_distribution_cb_area(cbarea1, cbarea2):
    ...
    #
    yearstart = config["startdate"][:4]
    yearend = config["enddate"][:4]
    fig, ax = plt.subplots()
    ...
    ax.set_xticklabels(
        [f"{el:3.1f}" for el in [el * 10 for el in np.arange(0.25, 2.5, 0.25)]]
    )
    ax.set(
        xlabel=r"Cloud band area (10$^6\,$m$^2$)",
        ylabel=r"Standardized distribution (10$^{-2}$)",
    )
...
if __name__ == "__main__":
    ...
    config["flag_djfm"] = True
    list_of_cloud_bandsSP_djfm = load_multiyears_data(
        config, varname="list_of_cloud_bands"
    )
    # flatten list of cloud band areas
    cb_areaSP = [cb.area for sublist in list_of_cloud_bandsSP for cb in sublist]
    cb_areaSP_djfm = [
        cb.area for sublist in list_of_cloud_bandsSP_djfm for cb in sublist
    ]
    fig = plot_distribution_cb_area(cb_areaSP, cb_areaSP_djfm)
    fig.show()
    figurename = f"{config['dir_figures']}/distribution_cb_area_{config['startdate'][:4]}_{config['enddate'][:4]}_fullperiod_vs_djfm_{config['domain']}.png"
